...
model = dict(
    ...
    backbone=dict(
        ...
        base_net=dict(
            ...
            pre_weights='pretrained/convnextv2_base_22k_384_ema.pt'
        ),
        hr_adapter=dict(
            type='HRAdapter',
            norm=norm,
            act=act,
            transitions=dict(
                trans1=dict(base_channels=128, hr_channels=64, ratio=1),
                trans2=dict(base_channels=256, hr_channels=64, ratio=1),
                trans3=dict(base_channels=512, hr_channels=128, ratio=1),
                trans4=dict(base_channels=1024, hr_channels=256, ratio=1),
            ),
            hr_modules=dict(
                stage1=dict(
                    num_modules=1,
...
trainer = dict(
    ...
    criterion=dict(
        ...
        loss_oks=dict(type='OksLoss', use_bbox=True, weight=1e-3),
