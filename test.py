...
def parse_args():
    ...
    parser.add_argument('--master_port', type=str, default='4321',
...
def main_worker(gpu, cfg, args, results_queue=None):
    ...
    data_loader = build_val_loader(dataset, num_workers=1, distributed=args.distributed)
    logger.info('Validation data loader: there are {} samples in each loader'.format(len(data_loader)))

    # init model
    logger.info('init model...')
    model = build_net(cfg.model)
    ...
    # evaluate
    logger.info('testing...')
    results = evaluate(model, data_loader, cfg.eval_cfg, gpu_id, args.distributed)
...
if __name__ == '__main__':
    ...
    if 'pre_weights' in cfg.model:
        cfg.model.pop('pre_weights')
    if 'pre_weights' in cfg.model.backbone.base_net:
        cfg.model.backbone.base_net.pop('pre_weights')
    if 'pre_weights' in cfg.model.backbone.hr_adapter:
        cfg.model.backbone.hr_adapter.pop('pre_weights')
